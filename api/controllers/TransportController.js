...
module.exports = {
    add: (req, res) => {
        let params = req.body;
        Transport.create(params).exec((err, result) => {
            if (err) return res.send(Message.messages(0, '创建失败！', err));
            // if (!result.length) return res.send(Message.messages(0, '创建失败！',result));
            res.send(Message.messages(1, '创建成功', result));
        })
    },
    delete: (req, res) => {
        let params = req.body;
        Transport.destroy(params).exec((err, result) => {
    ...
    update: (req, res) => {
        ...
        Transport.update(params, updateParams).exec((err, result) => {
    ...
    find: (req, res) => {
        ...
        Transport.find(params).exec((err, result) => {
